...
/**
 * The View component renders a dynamic view of data based on user selection, including functionalities
 * such as search, pagination, and sorting. It controls access based on user roles and allows deletion
 * of items with appropriate permissions.
 *
 * @param {Object} params - Parameters passed to the component, including the specific view to render.
...
const View = ({ params }) => {
	...
	// Filter the data to include only selected columns for the current view
	const selectedData = columns[view]
		? data.results.map((item) => {
				const selectedColumns = {};
				columns[view].forEach((column) => {
					selectedColumns[column] = item[column];
				});
	...
	const handleDeleteConfirm = async () => {
		try {
			...
			const newUrl = `/sensors/?page=${newPage + 1}&page_size=${rowsPerPage}`;
			mutate(newUrl);
		} catch (error) {
	...
	const handleOnSearch = (query) => {
		...
		setPage(0);
		const searchUrl = `/sensors/?page=1&page_size=${rowsPerPage}&search=${query}`;
		mutate(searchUrl);
	};
